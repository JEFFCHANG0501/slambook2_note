...
int main() {
  ...
  Vector3d v(1, 0, 0);
  // Rotate a vector using AngleAxis
  ...
  Vector3d euler_angles =
      rotation_matrix.eulerAngles(2, 1, 0);  // ZYX order, i.e. yaw pitch roll
  cout << "yaw pitch roll = " << euler_angles.transpose() << endl;

  Isometry3d T = Isometry3d::Identity();
  cout << "Transform matrix(Identity) = \n" << T.matrix() << endl;
  ...
  cout << "quaternion from rotation vector = \n"
       << q.coeffs().transpose()
       << endl;  // Note the order is (x,y,z,w), w is the real part
